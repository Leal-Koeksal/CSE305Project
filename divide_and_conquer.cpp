...
void threaded_evaluate(Node* node, std::promise<double> result_promise) {
    try {
        if (!node) throw std::runtime_error("Null node encountered");
        ...
        std::thread left_thread(threaded_evaluate, node->getLeftChild(), std::move(left_promise));
        std::thread right_thread(threaded_evaluate, node->getRightChild(), std::move(right_promise));
...
double evaluate_parallel(Node* node) {
    ...
    std::thread main_thread(threaded_evaluate, node, std::move(result_promise));
    main_thread.join();
